...
def learn_IKD(x):
    ...
    start = time.time()
    if f == 'GP':
        z_ikd = core.ikd_blockwise(cov_samp_th, d_latent, variance=variance_samp, clique_th_or_d_observation=0.4, max_n_cliques=2)
    else:
        z_ikd = core.ikd_blockwise(cov_samp_th, d_latent, variance=variance_samp, clique_th_or_d_observation=0.2, max_n_cliques=2)
    end = time.time()
